...
const API_KEY = '55f44f69542bdcc92ba9ef0343f77bef';
...
async function getWeather() {
    try {
        ...
        if (!res.ok) {
            console.error('an error occured: status: ' + res.status + error)
            return;
        }
...
function displayData(data) {
    display.innerHTML = `Location: <b>${data.name}</b> <br>`;
    ...
    display.innerHTML += `Wind Gust: <b>${data.wind.gust}</b> m/s <br>`;
    display.innerHTML += `Wind Speed: <b>${data.wind.speed}</b> m/s <br>`;
